...
export class ReportHelper {
  ...
  static dailyDataBlock(
    data: BaseDataPoint[],
    report: BaseForecastReport
  ): DailyDataBlock {
    if (!data.length) {
    ...
    const dailyDataBlock: DailyDataBlock = {
      ...
      data: ReportHelper.splitByDay(data, report.timezone).map((item) =>
        ReportHelper.dailyDataPoint(item, report)
      )
    };
  ...
  static hoursDataBlock(data: HourlyDataPoint[]): HoursDataBlock {
    ...
    const dataBlock: HoursDataBlock = {
      icon: ReportHelper.mostPopularIcon(data),
      data: ReportHelper.splitByDayPeriod(data).map((item) =>
        ReportHelper.hoursDataPoint(item)
      )
    };
  ...
  static hoursDataPoint(data: HourlyDataPoint[]): HoursDataPoint {
    ...
    const tempData = data.reduce((prev, current) => {
      ...
      if (high) {
        if (prev.high === undefined || high > prev.high) {
          ...
          if ((<HoursDataPoint>current).temperatureHighTime) {
            prev.highTime = (<HoursDataPoint>current)
              .temperatureHighTime as number;
          }
        }
      } else {
      ...
      if (low) {
        if (prev.low === undefined || low < prev.low) {
          ...
          if ((<HoursDataPoint>current).temperatureLowTime) {
            prev.lowTime = (<HoursDataPoint>current)
              .temperatureLowTime as number;
          }
        }
      } else {
  ...
  static mostPopularIcon(data: BaseDataPoint[]): ForecastIcon {
    ...
    let mostPopularIcon: ForecastIcon =
      data[parseInt((data.length / 2).toString())].icon;
  ...
  static convertDataPointUnits<T extends BaseDataPoint>(
    data: T,
    currentUnits: ForecastUnits,
    newUnits: ForecastUnits
  ): T {
  ...
  static splitByDayPeriod(data: HoursDataPoint[]) {
    ...
    data.forEach((item) => {
      if (
        currentData.length &&
        ~[0, 6, 12, 18].indexOf(new Date(item.time * 1000).getUTCHours())
      ) {
        list.push(currentData);
        currentData = [];
      }
