...
import {
  getDataPointIdByProp,
  HOURS_DATA_POINT_PROPS,
  HOURLY_DATA_POINT_PROPS
...
export class DataBlockMinifier {
  static toDetails(data: string): HoursDataBlock {
    const dataBlock = decompressDataBlock<HoursDataBlock>(
      data,
      Object.keys(HOURS_DATA_POINT_PROPS)
    );
    return dataBlock;
  }
  static toHourly(data: string): HourlyDataBlock {
    const dataBlock = decompressDataBlock<HourlyDataBlock>(
      data,
      Object.keys(HOURLY_DATA_POINT_PROPS)
    );
    return dataBlock;
  }
  static fromHourly(data: HourlyDataBlock): string {
    const dataBlock = compressDataBlock(
      data,
      Object.keys(HOURLY_DATA_POINT_PROPS)
    );
    return dataBlock;
...
function decompressDataBlock<T extends BaseDataBlock>(
  data: string,
  props: string[]
): T {
