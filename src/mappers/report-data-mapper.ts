...
import {
  DETAILS_REPORT_EXPIRES_IN_HOURS,
  HOURLY_REPORT_EXPIRES_IN_HOURS,
  unixTime
} from "../utils";
...
export class ReportDataMapper {
  static fromDetails(
    segment: HoursDataBlock,
    report: BaseForecastReport
  ): ReportData {
    const createdAt = new Date();
    return {
      expiresAt: unixTime(
        new Date(Date.now() + DETAILS_REPORT_EXPIRES_IN_HOURS * 60 * 60 * 1000)
      ),
  ...
  static fromHourly(
    segment: HourlyDataBlock,
    report: BaseForecastReport
  ): ReportData {
    const createdAt = new Date();
    return {
      expiresAt: unixTime(
        new Date(Date.now() + HOURLY_REPORT_EXPIRES_IN_HOURS * 60 * 60 * 1000)
      ),
      createdAt: createdAt.toISOString(),
